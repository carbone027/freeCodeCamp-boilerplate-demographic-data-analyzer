...
def calculate_demographic_data(print_data=True):
    ...
    # 1. How many of each race are represented in this dataset? (race column)
    ...
    country_group = df[df['salary'] == '>50K'].groupby('native-country')['native-country'].count() / df.groupby('native-country')['native-country'].count()
    highest_earning_country = country_group.idxmax()
    highest_earning_country_percentage = round(country_group.max() * 100, 1)
